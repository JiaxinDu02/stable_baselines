...
class DQN(OffPolicyRLModel):
    ...
    def learn(self, total_timesteps, callback=None, log_interval=100, tb_log_name="DQN",
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            reset = True
            obs = self.env.reset(return_info=False)
            ...
            for _ in range(total_timesteps):
                ...
                with self.sess.as_default():
                    # print("feed", np.array(obs)[None], type(np.array(obs)[None]))
                    # print("-------------")
                    # print(len(np.array(obs)[None]))
                    # for i in np.array(obs)[None]:
                    #     print(i)
                    # print("----")
                    # print(np.array(obs)[None].shape)
                    action = self.act(np.array(obs)[None], update_eps=update_eps, **kwargs)[0]
                env_action = action
                reset = False
                new_obs, rew, _, done, info = self.env.step(env_action)
