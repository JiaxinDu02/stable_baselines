        ...
        with tf.compat.v1.variable_scope("model", reuse=reuse):
            with tf.compat.v1.variable_scope("action_value"):
                if feature_extraction == "cnn":
                    ...
                    action_out = extracted_features
                    for layer_size in layers: # tf.compat.v1.layers.
                        action_out = tf.compat.v1.layers.dense(action_out, layer_size, activation=None)
                        if layer_norm:
                            action_out = tf_layers.layer_norm(action_out, center=True, scale=True)
                        action_out = act_fun(action_out)

                action_scores = tf.compat.v1.layers.dense(action_out, self.n_actions, activation=None)

            if self.dueling:
                with tf.compat.v1.variable_scope("state_value"):
                    state_out = extracted_features
                    for layer_size in layers:
                        state_out = tf.compat.v1.layers.dense(state_out, layer_size, activation=None)
                        if layer_norm:
